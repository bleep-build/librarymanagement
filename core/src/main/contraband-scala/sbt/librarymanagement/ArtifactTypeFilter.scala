...
/** Work around the inadequacy of Ivy's ArtifactTypeFilter (that it cannot reverse a filter)
  * @param types
  *   Represents the artifact types that we should try to resolve for (as in the allowed values of `artifact[type]` from a dependency `<publications>` section).
  *   One can use this to filter source / doc artifacts.
  * @param inverted
  *   Whether to invert the types filter (i.e. allow only types NOT in the set)
  */
final class ArtifactTypeFilter private (val types: Set[String], val inverted: Boolean)
    extends bleep.nosbt.librarymanagement.ArtifactTypeFilterExtra
    with Serializable {
