...
trait CallerFormats { self: bleep.nosbt.librarymanagement.ModuleIDFormats & bleep.nosbt.librarymanagement.ConfigRefFormats & sjsonnew.BasicJsonProtocol =>
    ...
    override def read[J](__jsOpt: Option[J], unbuilder: Unbuilder[J]): bleep.nosbt.librarymanagement.Caller =
      __jsOpt match {
        case Some(__js) =>
          ...
          unbuilder.endObject()
          bleep.nosbt.librarymanagement.Caller(
            caller,
