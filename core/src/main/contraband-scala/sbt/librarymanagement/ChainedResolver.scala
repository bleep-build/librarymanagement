...
final class ChainedResolver private (name: String, val resolvers: Vector[bleep.nosbt.librarymanagement.Resolver])
    extends bleep.nosbt.librarymanagement.Resolver(name)
    with Serializable {
  ...
  private def copy(name: String = name, resolvers: Vector[bleep.nosbt.librarymanagement.Resolver] = resolvers): ChainedResolver =
