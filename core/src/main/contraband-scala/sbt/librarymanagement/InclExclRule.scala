...
/** Defines a rule to either: <ul> <li>Exclude unwanted dependencies pulled in transitively by a module, or to</li> <li>Include and merge artifacts coming from
  * the ModuleDescriptor if "dependencyArtifacts" are also provided.</li> </ul> The use case that is applied depends on the parameter name which it is passed
  * to, but the filter has the same fields in both cases.
  */
final class InclExclRule private (
  ...
  override def equals(o: Any): Boolean = this.eq(o.asInstanceOf[AnyRef]) || (o match {
    case x: InclExclRule =>
      (this.organization == x.organization) && (this.name == x.name) && (this.artifact == x.artifact) && (this.configurations == x.configurations) && (this.crossVersion == x.crossVersion)
    case _ => false
  })
...
object InclExclRule extends bleep.nosbt.librarymanagement.InclExclRuleFunctions {
  ...
  ): InclExclRule = new InclExclRule(organization, name, artifact, configurations, crossVersion)
}
