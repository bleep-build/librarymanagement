...
object IvyFileConfiguration {
  ...
  ): IvyFileConfiguration = new IvyFileConfiguration(validate, scalaModuleInfo, file, autoScalaTools)
  def apply(
      validate: Boolean,
      scalaModuleInfo: bleep.nosbt.librarymanagement.ScalaModuleInfo,
      file: java.io.File,
      autoScalaTools: Boolean
  ): IvyFileConfiguration = new IvyFileConfiguration(validate, Option(scalaModuleInfo), file, autoScalaTools)
}
