...
final class MavenRepo private (name: String, root: String, localIfFile: Boolean, val _allowInsecureProtocol: Boolean)
    extends bleep.nosbt.librarymanagement.MavenRepository(name, root, localIfFile)
    with Serializable {
  ...
  override def equals(o: Any): Boolean = this.eq(o.asInstanceOf[AnyRef]) || (o match {
    case x: MavenRepo =>
      (this.name == x.name) && (this.root == x.root) && (this.localIfFile == x.localIfFile) && (this._allowInsecureProtocol == x._allowInsecureProtocol)
    case _ => false
  })
  ...
  private def copy(
      name: String = name,
      root: String = root,
      localIfFile: Boolean = localIfFile,
      _allowInsecureProtocol: Boolean = _allowInsecureProtocol
  ): MavenRepo =
