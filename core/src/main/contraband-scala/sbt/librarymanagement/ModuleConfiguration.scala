...
final class ModuleConfiguration private (val organization: String, val name: String, val revision: String, val resolver: bleep.nosbt.librarymanagement.Resolver)
    extends Serializable {

  override def equals(o: Any): Boolean = this.eq(o.asInstanceOf[AnyRef]) || (o match {
    case x: ModuleConfiguration =>
      (this.organization == x.organization) && (this.name == x.name) && (this.revision == x.revision) && (this.resolver == x.resolver)
    case _ => false
  })
  ...
  private def copy(
      ...
      resolver: bleep.nosbt.librarymanagement.Resolver = resolver
  ): ModuleConfiguration =
...
object ModuleConfiguration {
  ...
  def apply(organization: String, name: String, revision: String, resolver: bleep.nosbt.librarymanagement.Resolver): ModuleConfiguration =
    new ModuleConfiguration(organization, name, revision, resolver)
}
