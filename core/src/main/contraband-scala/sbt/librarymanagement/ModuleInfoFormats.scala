...
trait ModuleInfoFormats { self: bleep.nosbt.librarymanagement.ScmInfoFormats & bleep.nosbt.librarymanagement.DeveloperFormats & sjsonnew.BasicJsonProtocol =>
    ...
    override def read[J](__jsOpt: Option[J], unbuilder: Unbuilder[J]): bleep.nosbt.librarymanagement.ModuleInfo =
      __jsOpt match {
        case Some(__js) =>
          ...
          unbuilder.endObject()
          bleep.nosbt.librarymanagement.ModuleInfo(
            nameFormal,
            description,
            homepage,
            startYear,
