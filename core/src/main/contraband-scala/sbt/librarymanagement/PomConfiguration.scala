...
final class PomConfiguration private (
  ...
  override def equals(o: Any): Boolean = this.eq(o.asInstanceOf[AnyRef]) || (o match {
    case x: PomConfiguration =>
      (this.validate == x.validate) && (this.scalaModuleInfo == x.scalaModuleInfo) && (this.file == x.file) && (this.autoScalaTools == x.autoScalaTools)
    case _ => false
  })
  ...
  private def copy(
      ...
      autoScalaTools: Boolean = autoScalaTools
  ): PomConfiguration =
...
object PomConfiguration {
  ...
  def apply(
      ...
      scalaModuleInfo: Option[bleep.nosbt.librarymanagement.ScalaModuleInfo],
      file: java.io.File,
      autoScalaTools: Boolean
  ): PomConfiguration = new PomConfiguration(validate, scalaModuleInfo, file, autoScalaTools)
  def apply(validate: Boolean, scalaModuleInfo: bleep.nosbt.librarymanagement.ScalaModuleInfo, file: java.io.File, autoScalaTools: Boolean): PomConfiguration =
    new PomConfiguration(validate, Option(scalaModuleInfo), file, autoScalaTools)
}
