...
final class SftpRepository private (name: String, patterns: bleep.nosbt.librarymanagement.Patterns, connection: bleep.nosbt.librarymanagement.SshConnection)
    extends bleep.nosbt.librarymanagement.SshBasedRepository(name, patterns, connection)
    with bleep.nosbt.librarymanagement.SftpRepositoryExtra
    with Serializable {
...
object SftpRepository {
  def apply(name: String, connection: bleep.nosbt.librarymanagement.SshConnection, patterns: bleep.nosbt.librarymanagement.Patterns) =
    new SftpRepository(name, patterns, connection)
  def apply(name: String, patterns: bleep.nosbt.librarymanagement.Patterns, connection: bleep.nosbt.librarymanagement.SshConnection): SftpRepository =
    new SftpRepository(name, patterns, connection)
}
