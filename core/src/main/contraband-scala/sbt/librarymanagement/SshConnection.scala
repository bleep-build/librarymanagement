...
final class SshConnection private (
  ...
  private def copy(
      authentication: Option[bleep.nosbt.librarymanagement.SshAuthentication] = authentication,
...
object SshConnection {
  ...
  def apply(authentication: Option[bleep.nosbt.librarymanagement.SshAuthentication], hostname: Option[String], port: Option[Int]): SshConnection =
    new SshConnection(authentication, hostname, port)
  def apply(authentication: bleep.nosbt.librarymanagement.SshAuthentication, hostname: String, port: Int): SshConnection =
    new SshConnection(Option(authentication), Option(hostname), Option(port))
}
