...
final class URLRepository private (name: String, patterns: bleep.nosbt.librarymanagement.Patterns, val allowInsecureProtocol: Boolean)
    extends bleep.nosbt.librarymanagement.PatternsBasedRepository(name, patterns)
    with Serializable {
...
object URLRepository {
  ...
  def apply(name: String, patterns: bleep.nosbt.librarymanagement.Patterns, allowInsecureProtocol: Boolean): URLRepository =
    new URLRepository(name, patterns, allowInsecureProtocol)
}
