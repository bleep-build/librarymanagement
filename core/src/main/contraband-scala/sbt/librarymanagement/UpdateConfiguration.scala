...
/** @param retrieveManaged
  *   If set to some RetrieveConfiguration, this enables retrieving dependencies to the specified directory. Otherwise, dependencies are used directly from the
  *   cache.
  * @param missingOk
...
final class UpdateConfiguration private (
  ...
  private def this() =
    this(None, false, bleep.nosbt.librarymanagement.UpdateLogging.Default, bleep.nosbt.librarymanagement.LogicalClock.unknown, None, None, false, false)

  override def equals(o: Any): Boolean = this.eq(o.asInstanceOf[AnyRef]) || (o match {
    case x: UpdateConfiguration =>
      (this.retrieveManaged == x.retrieveManaged) && (this.missingOk == x.missingOk) && (this.logging == x.logging) && (this.logicalClock == x.logicalClock) && (this.metadataDirectory == x.metadataDirectory) && (this.artifactFilter == x.artifactFilter) && (this.offline == x.offline) && (this.frozen == x.frozen)
    case _ => false
  })
  ...
  private def copy(
      retrieveManaged: Option[bleep.nosbt.librarymanagement.RetrieveConfiguration] = retrieveManaged,
      missingOk: Boolean = missingOk,
      logging: bleep.nosbt.librarymanagement.UpdateLogging = logging,
      logicalClock: bleep.nosbt.librarymanagement.LogicalClock = logicalClock,
      metadataDirectory: Option[java.io.File] = metadataDirectory,
      artifactFilter: Option[bleep.nosbt.librarymanagement.ArtifactTypeFilter] = artifactFilter,
...
object UpdateConfiguration {
  ...
  def apply(
      retrieveManaged: Option[bleep.nosbt.librarymanagement.RetrieveConfiguration],
      missingOk: Boolean,
      logging: bleep.nosbt.librarymanagement.UpdateLogging,
      logicalClock: bleep.nosbt.librarymanagement.LogicalClock,
      metadataDirectory: Option[java.io.File],
      artifactFilter: Option[bleep.nosbt.librarymanagement.ArtifactTypeFilter],
      offline: Boolean,
      frozen: Boolean
  ): UpdateConfiguration = new UpdateConfiguration(retrieveManaged, missingOk, logging, logicalClock, metadataDirectory, artifactFilter, offline, frozen)
  def apply(
      retrieveManaged: bleep.nosbt.librarymanagement.RetrieveConfiguration,
      missingOk: Boolean,
      logging: bleep.nosbt.librarymanagement.UpdateLogging,
      logicalClock: bleep.nosbt.librarymanagement.LogicalClock,
      metadataDirectory: java.io.File,
      artifactFilter: bleep.nosbt.librarymanagement.ArtifactTypeFilter,
      offline: Boolean,
      frozen: Boolean
  ): UpdateConfiguration =
    new UpdateConfiguration(Option(retrieveManaged), missingOk, logging, logicalClock, Option(metadataDirectory), Option(artifactFilter), offline, frozen)
}
