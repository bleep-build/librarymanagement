...
/** Provides information about dependency resolution. It does not include information about evicted modules, only about the modules ultimately selected by the
  * conflict manager. This means that for a given configuration, there should only be one revision for a given organization and module name.
  * @param cachedDescriptor
...
final class UpdateReport private (
    ...
    val stamps: Map[java.io.File, Long]
) extends bleep.nosbt.librarymanagement.UpdateReportExtra
    with Serializable {

  override def equals(o: Any): Boolean = this.eq(o.asInstanceOf[AnyRef]) || (o match {
    case x: UpdateReport =>
      (this.cachedDescriptor == x.cachedDescriptor) && (this.configurations == x.configurations) && (this.stats == x.stats) && (this.stamps == x.stamps)
    case _ => false
  })
  ...
  private def copy(
      cachedDescriptor: java.io.File = cachedDescriptor,
      configurations: Vector[bleep.nosbt.librarymanagement.ConfigurationReport] = configurations,
      stats: bleep.nosbt.librarymanagement.UpdateStats = stats,
      stamps: Map[java.io.File, Long] = stamps
  ): UpdateReport =
...
object UpdateReport {
  ...
  def apply(
      ...
      configurations: Vector[bleep.nosbt.librarymanagement.ConfigurationReport],
      stats: bleep.nosbt.librarymanagement.UpdateStats,
      stamps: Map[java.io.File, Long]
  ): UpdateReport = new UpdateReport(cachedDescriptor, configurations, stats, stamps)
}
