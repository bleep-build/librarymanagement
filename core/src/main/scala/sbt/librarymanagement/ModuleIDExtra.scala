...
private[librarymanagement] abstract class ModuleIDExtra {
  ...
  /** Specifies the cross-version behavior for this module. See [CrossVersion] for details. Unlike `withCrossVersion(...)`, `cross(...)` will preserve the
    * prefix and suffix values from the existing `crossVersion` value.
    *
    * {{{
  ...
  /** Marks this dependency as "changing". Ivy will always check if the metadata has changed and then if the artifact has changed, redownload it. sbt configures
    * all -SNAPSHOT dependencies to be changing.
    *
  ...
  /** Indicates that conflict resolution should only select this module's revision. This prevents a newer revision from being pulled in by a transitive
  ...
  /** Specifies a URL from which the main artifact for this dependency can be downloaded. This value is only consulted if the module is not found in a
    * repository. It is not included in published metadata.
    */
  ...
  /** Declares the explicit artifacts for this module. If this ModuleID represents a dependency, these artifact definitions override the information in the
    * dependency's published metadata.
    */
  def artifacts(newArtifacts: Artifact*): ModuleID =
    withExplicitArtifacts(newArtifacts.toVector ++ explicitArtifacts)

  /** Applies the provided exclusions to dependencies of this module. Note that only exclusions that specify both the exact organization and name and nothing
    * else will be included in a pom.xml.
    */
  ...
  /** Adds extra attributes for this module. All keys are prefixed with `e:` if they are not already so prefixed. This information will only be published in an
    * ivy.xml and not in a pom.xml.
    */
  def extra(attributes: (String, String)*): ModuleID =
    withExtraAttributes(extraAttributes ++ ModuleID.checkE(attributes))

  /** Not recommended for new use. This method is not deprecated, but the `update-classifiers` task is preferred for performance and correctness. This method
    * adds a dependency on this module's artifact with the "sources" classifier. If you want to also depend on the main artifact, be sure to also call `jar()`
    * or use `withSources()` instead.
    */
  def sources(): ModuleID = artifacts(Artifact.sources(name))

  /** Not recommended for new use. This method is not deprecated, but the `update-classifiers` task is preferred for performance and correctness. This method
    * adds a dependency on this module's artifact with the "javadoc" classifier. If you want to also depend on the main artifact, be sure to also call `jar()`
    * or use `withJavadoc()` instead.
    */
  ...
  /** Not recommended for new use. This method is not deprecated, but the `update-classifiers` task is preferred for performance and correctness. This method
    * adds a dependency on this module's artifact with the "sources" classifier. If there is not already an explicit dependency on the main artifact, this adds
    * one.
    */
  def withSources(): ModuleID = jarIfEmpty.sources()

  /** Not recommended for new use. This method is not deprecated, but the `update-classifiers` task is preferred for performance and correctness. This method
    * adds a dependency on this module's artifact with the "javadoc" classifier. If there is not already an explicit dependency on the main artifact, this adds
    * one.
    */
  ...
  /** Declares a dependency on the main artifact. This is implied by default unless artifacts are explicitly declared, such as when adding a dependency on an
